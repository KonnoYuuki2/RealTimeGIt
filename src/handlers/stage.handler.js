...
export const moveStageHandler = (userId, payload) => { //userId = uuid
    ...
    for(let key in stages.data) {
      if(stages.data[key].id === currentStage.id) {
        savekey = key;
      }
    } 
    ...
    // 5 => 임의로 정한 오차범위
    if(elapsedTime === stages.data[+savekey + 1].score && elapsedTime > stages.data[+savekey + 1].score + 5 ) { // 다음 스테이지로 넘어가기 위해 필요한 점수가 100이고 그에 해당하는 값이 되면 자동으로 넘어간다.
        return {status: 'fail', message: 'Invalid elapsed time'}
    }


    // 배열을 구성하는 값 중에서 조건이 하나라도 맞을 경우 -> some
    if(!stages.data.some((stage) => stage.id === payload.targetStage)) { // 다음 스테이지에 있는 id가 stage.json에 존재하는가?
